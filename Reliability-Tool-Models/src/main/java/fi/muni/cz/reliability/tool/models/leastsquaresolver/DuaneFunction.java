 ...
 */
public class DuaneFunction implements Function {
    ...
    public MultivariateVectorFunction getMultivariateVectorFunction() {
        return (double[] variables) -> {
            ...
            for (int i = 0; i < values.length; ++i) {
                values[i] = variables[0] *  Math.pow(i, variables[1]);
            }
            return values;
        };
    }
        ...
        return new MultivariateMatrixFunction() {
            ...
            private double[][] jacobian(double[] variables) {
                ...
                for (int i = 0; i < jacobian.length; ++i) {
                    jacobian[i][0] = Math.pow(x.get(i), variables[1]);
                    jacobian[i][1] = variables[0] *  Math.pow(x.get(i), variables[1]) * Math.log(x.get(i));
                }
                return jacobian;
            }
        };
